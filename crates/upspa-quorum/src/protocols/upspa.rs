...
pub fn make_fixture(nsp: usize, tsp: usize) -> Fixture {
    ...
    let one_ciphersp = crypto::xchacha_encrypt_detached(&fk, &ciphersp_aad, &ciphersp_pt, &mut rng);
...
fn recover_state_and_cipherid_pt_user_side(
    ...
    black_box(b);
    let oprf_out = crypto::toprf_client_eval(&fx.password, it.r, &it.partials, &fx.lagrange_at_zero);
    let state_key: [u8; 32] = oprf_out;
...
pub fn secret_update_user_side(fx: &Fixture, it: &IterData) -> [u8; 32] {
    ...
    for j in 0..m {
        ...
        let pt =
            crypto::xchacha_decrypt_detached(&fk, &fx.ciphersp_aad, blob).expect("ciphersp must decrypt");
        let mut rlsj = [0u8; 32];
...
fn recover_state_user_side(fx: &Fixture, it: &IterData) -> ([u8; 32], [u8; 32], SigningKey) {
    ...
    black_box(b);
    let oprf_out =
        crypto::toprf_client_eval_from_partials(&fx.password, it.r, &it.partials, &fx.lagrange_at_zero);
    let state_key: [u8; 32] = oprf_out;
    let pt = crypto::xchacha_decrypt_detached(&state_key, &fx.cipherid_aad, &fx.cipherid)
        .expect("cipherid must decrypt if TOPRF is correct");
    let mut sid_bytes = [0u8; 32];
